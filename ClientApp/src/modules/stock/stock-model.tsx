...
export default class Stock implements SimpleModel, Entity {
  id!: number | null
  name!: string
  createdAt?: Date
  updatedAt?: Date
  ...
  constructor(data?: SimpleModel) {
    ...
    this.service = new ModelService<SimpleModel>('Stocks')
  }
  ...
  title(type: 'list' | 'form') {
    switch (type) {
      case 'list':
        return 'Locais de estoque'
      case 'form':
        if (!this.id) return 'Novo local de estoque'

        return 'Editar local de estoque'
    }
  }
